...
listener.subscribe(function (message) {
    ...
    let timestamp = `${message.header.seq}`;
    ...
    xvizServer.updateLocation(latitude, longitude, altitude, 0, 0, 0, x_dir_velocity ,steering_degree, x_dir_acl, parseFloat(timestamp));
});
...
listener4.subscribe(function (message) {
    ...
    car_heading_utm_north = message.heading;
...
listener5.subscribe(function (message){
  let {acceleration,velocity,steering} = message;
  x_dir_acl = acceleration;
  x_dir_velocity = velocity* 3.6; // m/s -> km/h
  steering_degree = radToDegree(steering);
  console.log(x_dir_acl,x_dir_velocity,steering_degree)
});
